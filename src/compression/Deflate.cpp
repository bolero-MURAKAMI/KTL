...
static void regist_ktl_deflate() {
	...
	if (global) {
		sprig::krkr::tjs::AddNewMember(global.get(), SPRIG_KRKR_TJS_W("Deflate"), ktl::CreateNativeClassDeflate());
	}
}
static void unregist_ktl_deflate() {
	...
	sprig::krkr::tjs::object_type global(::TVPGetScriptDispatch(), false);
	if (global) {
		sprig::krkr::tjs::DeleteMember(global.get(), SPRIG_KRKR_TJS_W("Deflate"));
	}
}
